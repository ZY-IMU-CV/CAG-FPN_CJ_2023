...
def main(args):
    ...
    if eval_types:
        ...
        if eval_types == ['proposal_fast']:
            ...
            # 2021.1.13 by ynh
            recall_result, precision_result, coco_recall_result, coco_precision_result = coco_eval(result_file,
                                                                                                   eval_types, coco)
        else:
            if not isinstance(results[0], dict):
                ...
                # 2021.1.13 by ynh
                recall_result, precision_result, coco_recall_result, coco_precision_result = coco_eval(result_files,
                                                                                                       eval_types, coco)
            else:
    ...
    for n_key in range(imgId_num):
        ...
        # 2021.1.13 by ynh
        if (0 < precision < 1):
            out_file = os.path.join(args.output_vis_result, "precision", "badcase",
                                    'precision_result_{}.jpg'.format(vis_idx))
        elif (precision == -1):
            out_file = os.path.join(args.output_vis_result, "precision", "abnormalcase",
                                    'result_{}.jpg'.format(vis_idx))
        else:
